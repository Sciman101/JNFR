  ...
  argTree: branch([
    literal(
      ...
      discordMention(
        "channel",
        "channel",
        discordEmoji("emoji", numValue("min_reacts", 1, 500, true))
      )
    ),
    literal("disable"),
  ]),
        ...
        if (reaction.message.channel.id.toString() !== pinboard.channel) {
          ...
          if (match) {
            ...
            if (reaction.count >= pinboard.minReacts) {
              ...
              if (
                pinboard.cache.indexOf(reaction.message.id.toString()) == -1
              ) {
                ...
                if (channel) {
                  ...
                  if (message.attachments.size > 0) {
                    ...
                    //console.log(url);
                    const isVideo =
                      croppedUrl.endsWith("webm") ||
                      croppedUrl.endsWith("mp4") ||
                      croppedUrl.endsWith("mov");

                    pinEmbed.setImage(message.attachments.first().proxyURL);
                    if (isVideo) {
                      stupidWorkaroundForDiscordJsVideosNotEmbedding =
                    ...
                    if (message.embeds[0].thumbnail) {
                      ...
                      stupidWorkaroundForDiscordJsVideosNotEmbedding =
                        message.embeds[0].video.proxyURL;
                    }
                  }
                  ...
                  if (stupidWorkaroundForDiscordJsVideosNotEmbedding) {
                    ...
                    pinEmbed.setDescription(
                      `Discord.JS can't embed videos, so it's been posted above seperately`
                    );
                  }
                  ...
                  log.info(
                    "Pinned a message in channel id " + channel.id.toString()
                  );
                }
              }
            }
