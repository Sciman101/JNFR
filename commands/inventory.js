...
export default {
  ...
  async execute(message, args) {
    ...
    if (!args.itemname) {
      ...
      // Get inventory
      let lines = Object.values(inventory)
        ...
        .map(
          (slot) =>
            ...
            " (x" +
            slot.count +
            (items[slot.id].callbacks.used ? ", :gear:Usable:gear:" : "") +
            ")"
        );
...
\`You have: ${slot.count} | Total Owned: ${slot.owned} | Eaten: ${
        slot.eaten || 0
      }${item.callbacks.used ? ` | Used: ${slot.used || 0}` : ""}\`
${item.description}
			`;
