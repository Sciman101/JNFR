...
export default {
  ...
  execute(message, args) {
    ...
    if (args.command_name) {
      const command =
        commands.get(args.command_name) ||
        commands.find(
          (cmd) => cmd.aliases && cmd.aliases.includes(args.command_name)
        );
      if (command) {
...
`;
        ...
        message.reply(
          Babbler.get("unknown_command_help", { command: args.command_name })
        );
      }
    } else {
      // List all commands
      ...
      return message.author
        ...
        .catch((error) => {
          ...
          message.reply(
            "Couldn't DM you for some reason? Do you have DMs disabled?"
          );
        });
    }
  },
