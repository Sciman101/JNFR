  ...
  argTree: any(
    ...
    discordMention(
      ...
      stringValue(
        "message_id",
        false,
        discordEmoji("emoji", optional(discordMention("role", "role")))
      )
    )
  ),
  execute(message, args) {
    ...
    if (role === guild.roles.everyone) {
      return message.reply(
        `You can't make an emoji role for everyone! Don't be ridiculous`
      );
    }
  ...
  if (action == "add") {
    ...
    if (getEmojiRole(message, emoji)) {
      return commandMessage.reply(
        "This emoji is already configured for a role!"
      );
    }
    ...
    if (react) {
      ...
      const userReactions = message.reactions.cache.filter((reaction) =>
        reaction.users.cache.has("352566617231720468")
      );
      try {
